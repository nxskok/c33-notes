...
you'll also want `posterior` and `bayesplot`, from the same place:

```{r, eval=FALSE}
install.packages("posterior", repos = c("https://mc-stan.org/r-packages/", 
                                        getOption("repos")))
install.packages("bayesplot", repos = c("https://mc-stan.org/r-packages/", 
                                        getOption("repos")))
```
...
All of this is done once. If you have problems, go [here](https://mc-stan.org/cmdstanr/articles/cmdstanr.html).
...
- the posterior mean is 3.16
- with a 90% posterior interval of 2.21 to 4.30. 
...
```{r}
poisson2_draws <- as_draws_df(poisson2_fit$draws())
poisson2_draws
```
...
anova_fit <- anova_compiled, data = anova_data)
```
